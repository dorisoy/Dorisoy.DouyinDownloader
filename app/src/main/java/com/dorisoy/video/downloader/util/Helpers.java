package com.dorisoy.video.downloader.util;
...
public class Helpers {
    ...
    {
        return !StringUtils.isEmpty(extractVideoUrl(context, str));
    }

    /** 从抖音分享文案中提取首个完整链接，忽略其后的标点和说明文字。 */
    public static String extractVideoUrl(Context context, String shareText)
    {
        String url = stripUrl(shareText);
        if (StringUtils.isEmpty(url) || !url.matches(context.getString(R.string.video_url_regex)))
            return "";
