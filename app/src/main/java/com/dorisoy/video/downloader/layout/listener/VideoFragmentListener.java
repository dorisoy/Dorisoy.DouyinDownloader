package com.dorisoy.video.downloader.layout.listener;
...
import com.dorisoy.iconify.widget.IconTextView;

import com.dorisoy.video.downloader.GlideApp;
import com.dorisoy.video.downloader.MainActivity;
import com.dorisoy.video.downloader.R;
import com.dorisoy.video.downloader.bean.Video;
import com.dorisoy.video.downloader.content.analyzer.AnalyzerTask;
import com.dorisoy.video.downloader.core.io.Storage;
import com.dorisoy.video.downloader.core.listener.FragmentListener;
import com.dorisoy.video.downloader.layout.fragment.VideoFragment;
import com.dorisoy.video.downloader.util.io.FileStorage;
import com.dorisoy.video.downloader.util.network.DownloadQueue;
import com.dorisoy.video.downloader.util.network.DownloadTaskRegistry;
import com.dorisoy.video.downloader.util.network.Downloader;
    ...
    public void onAnalyzed(Video video, boolean fromHistory)
    {
        synchronized (VideoFragmentListener.class) {
            ...
            if (videoDownloader.getFile().exists()) // 文件已经下载, 则直接播放
                ...
                String taskId = "video-" + video.getId();
                downloadQueue.add(taskId, videoDownloader);
                DownloadTaskRegistry.put(taskId, video, videoDownloader);
...
/*            case "cover":
    ...
    public void onClose()
    {
        ...
        com.dorisoy.video.downloader.core.app.Process.background((Activity) context);
